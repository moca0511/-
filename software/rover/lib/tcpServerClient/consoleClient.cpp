...
void consoleTcpClient::printConsoleSupport(int dataType)
{
	...
	case 3:
		...
		break;
	case 4:
		printf("type, end Lat, end Lon, m, direction, queueAction \n");
		break;
	case 5:
...
void consoleTcpClient::char2tcpStruct(tcpStruct *data, const char str_in[])
{
	...
	else if (data->dataType == 4)
	{
		...
		data->useData.data4.recodType = std::stoi(pop(0));
		data->useData.data4.point.lat = std::stod(pop(1));
		data->useData.data4.point.lon = std::stod(pop(2));
		data->useData.data4.distination.m = std::stod(pop(3));
...
std::string consoleTcpClient::tcpStruct2string(tcpStruct *data)
{
	...
	case 0:
		...
		break;
	case 1:
		sprintf(temp_str, "%f", data->useData.data1.num2);
		break;
	case 2:
	...
	case 4:
		...
		sprintf(temp_str, "sendType=%d, e(lat,lon)=(%lf, %lf), (m, direction)=(%lf, %lf), [queue=%d]",
				data->useData.data4.recodType,
				data->useData.data4.point.lat, data->useData.data4.point.lon,
				data->useData.data4.distination.m, data->useData.data4.distination.direction,
				data->queueAction);
		break;
	case 5:
