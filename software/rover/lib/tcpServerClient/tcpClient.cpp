...
int myTcpConnect(const char *hostnm, const char *portnm, bool printFlg)
{
	char buf[512];
	...
	if (printFlg)
	{
		(void)fprintf(stderr, "tcpServer %s:%d\n", inet_ntop(AF_INET, &addr, buf, sizeof(buf)), ntohs(sa.sin_port));
	}
...
int clientRecv(int sd, void *buf, int bufSize, bool printFlg)
{
	int n;
